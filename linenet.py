...
def main(config,train_writer):
    ...
    for epoch in range(config.TRAIN.START_EPOCH, config.TRAIN.EPOCHS):
        ...
        if epoch%5==4:
            ...
            mapk, precision,hr10,hr50,r10_50,r10_100 = analysis(vec, [], DMVal, ComputeSimVal, knn, records, 100)
    ...
    knn = KNeighborsClassifier(n_neighbors=101)
    knn.fit(vec, range(len(vec)))
    analysis(vec, [], DMTest, ComputeSimTest, knn, records, 100)
...
def train_one_epoch_triplet(config, model, criterion_triplet, criterion_mse, dataset, train_loader, optimizer, epoch, lr_scheduler, status, name_map,NormDM,train_writer):
    ...
    for idx, (datas, names, series) in enumerate(train_loader):
        ...
        if config.TRAIN.TRIPLET_SELECTION=='diversified':
            ...
            samples=triplet_select_diversified(config,vec,triplet_cluster,series,names,status,name_map,datas,NormDM)
            tot=candidates=len(samples)
        else:
            tot=0
            candidates=len(names)

        for i in range(candidates//(triplet_batch)):
            ...
            if config.TRAIN.TRIPLET_SELECTION=='hard':
                sample=triplet_select_hard(config,vec,names,status,name_map,datas,names[i*triplet_batch:(i+1)*triplet_batch])
            elif config.TRAIN.TRIPLET_SELECTION=='semi_hard':
                sample=triplet_select_semi_hard(config,vec,names,status,name_map,datas,names[i*triplet_batch:(i+1)*triplet_batch])
            elif config.TRAIN.TRIPLET_SELECTION=='all':
                sample=triplet_select_all(config,names,status,name_map,datas,names[i*triplet_batch:(i+1)*triplet_batch])
            
            if config.TRAIN.TRIPLET_SELECTION=='diversified':
                sample = torch.cat(samples[int(i*triplet_batch):int((i+1)*triplet_batch)], 0).to(config.TRAIN.CUDA)
            else:
                tot+=len(sample)
                if len(sample)==0:
                    continue
